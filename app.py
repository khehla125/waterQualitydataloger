...
def read_data_from_sheet(device_id):
    try:
        sheet = client.open_by_key('1goJqD5eK8J6bayJlqC0RgEmxkDnmTOW6Jfzr3_cjwAQ').worksheet(device_id)
...
if data.empty:
    ...
    if data_filtered.empty:
        ...
        # Conductivity levels
        fig.add_trace(go.Scatter(x=data['Timestamp'], y=data['conductivity'], mode='lines+markers', name='conductivity'))

        # Turbidity levels
        fig.add_trace(
            go.Scatter(x=data['Timestamp'], y=data['turbidity'], mode='lines+markers', name='turbidity', yaxis='y2'))

        # Temperature levels
        fig.add_trace(
            go.Scatter(x=data['Timestamp'], y=data['temperature'], mode='lines+markers', name='temperature', yaxis='y3'))

        # Create axis objects
        fig.update_layout(
            xaxis=dict(domain=[0.1, 0.9]),
            yaxis=dict(title="Conductivity", titlefont=dict(color="#1f77b4"), tickfont=dict(color="#1f77b4")),
            yaxis2=dict(title="Turbidity", titlefont=dict(color="#ff7f0e"), tickfont=dict(color="#ff7f0e"), anchor="x",
                        overlaying="y", side="right", position=1),
            yaxis3=dict(title="Temperature (°C)", titlefont=dict(color="#d62728"), tickfont=dict(color="#d62728"),
